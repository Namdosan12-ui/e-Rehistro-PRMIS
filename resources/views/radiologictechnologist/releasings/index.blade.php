                    ...
                    <tr>
                        ...
                        <td>
                            @if ($releasing->result_file)
                                <a href="{{ route('radiologictechnologist.releasings.view', $releasing->id) }}" target="_blank">View Result</a>
                            @else
                                No result uploaded
                            @endif
                        </td>
                        <td>
                            @if (!$releasing->result_file)
                                <form action="{{ route('radiologictechnologist.releasings.upload', $releasing->id) }}" method="POST" enctype="multipart/form-data" class="upload-form">
                                    @csrf
                            ...
                            @elseif ($releasing->result_file && !$releasing->released_at)
                                @include('radiologictechnologist.partials.email-form', ['releasing' => $releasing])
                            @endif
                        </td>
                        <td>
                            @if (!$releasing->released_at)
                                @include('radiologictechnologist.partials.release_form', ['releasing' => $releasing])
                            @else
