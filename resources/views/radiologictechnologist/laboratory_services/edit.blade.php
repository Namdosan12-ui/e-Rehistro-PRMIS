...
<h2 class="all-header">
    ...
    {{ __('Edit Laboratory Service') }}
        ...
        <div class ="card-body">
            <form action="{{ route('radiologictechnologist.laboratory_services.update', $laboratoryService->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <a href="{{ route('radiologictechnologist.laboratory_services.index') }}" class="btn btn-cancel"> {{ __('Cancel') }}</a>
