                    ...
                    <tr>
                        ...
                        <td>
                            @if ($releasing->result_file)
                                <a href="{{ route('medicaltechnologist.releasings.view', $releasing->id) }}" target="_blank">View Result</a>
                            @else
                                No result uploaded
                            @endif
                        </td>
                        <td>
                            @if (!$releasing->result_file)
                                <form action="{{ route('medicaltechnologist.releasings.upload', $releasing->id) }}" method="POST" enctype="multipart/form-data" class="upload-form">
                                    @csrf
                            ...
                            @elseif ($releasing->result_file && !$releasing->released_at)
                                @include('medicaltechnologist.partials.email-form', ['releasing' => $releasing])
                            @endif
                        </td>
                        <td>
                            @if (!$releasing->released_at)
                                @include('medicaltechnologist.partials.release_form', ['releasing' => $releasing])
                            @else
                        ...
                        <td>
                            <form action="{{ route('medicaltechnologist.releasings.destroy', $releasing->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this releasing record?');">
                                @csrf
                                @method('DELETE')
