                    ...
                    <tr>
                        ...
                        <td>
                            @if ($releasing->result_file)
                                <a href="{{ route('reception.releasings.view', $releasing->id) }}" target="_blank">View Result</a>
                            @else
                                No result uploaded
                            @endif
                        </td>
                        <td>
                            @if (!$releasing->result_file)
                                <form action="{{ route('reception.releasings.upload', $releasing->id) }}" method="POST" enctype="multipart/form-data" class="upload-form">
                                    @csrf
                            ...
                            @elseif ($releasing->result_file && !$releasing->released_at)
                                @include('reception.partials.email-form', ['releasing' => $releasing])
                            @endif
                        </td>
                        <td>
                            @if (!$releasing->released_at)
                                @include('reception.partials.release_form', ['releasing' => $releasing])
                            @else
