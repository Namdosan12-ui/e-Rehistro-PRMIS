...
<form action="{{ route('reception.transactions.update', $transaction->id) }}" method="POST">
    @method('PUT')
    @csrf
    ...
    <div class="card">
                ...
                <div class="flex space-x-4">
                    ...
                    <div class="flex justify-end space-x-4 mt-4">
                        <button type="submit" class="px-6 py-3 bg-white text-blue-600 rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2">
                            <i class="fas fa-save"></i><span>Update Transaction</span>
                        </button>
                        <a href="{{ route('reception.transactions.show', $transaction->id) }}"
        ...
        <div class="all">
            <div class="section-container mb-8 bg-gray-50 rounded-2xl p-6 border border-gray-100">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="form-group">
                        ...
                        <select name="patient_id" id="patient_id" class="form-control1" required>
                            ...
                            @foreach($patients as $patient)
                                <option value="{{ $patient->id }}" {{ $transaction->patient_id == $patient->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="physician" id="physician" class="form-control1"
                               value="{{ old('physician', $transaction->physician) }}"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="radiologic_technologist" id="radiologic_technologist"
                               class="form-control1"
                               value="{{ old('radiologic_technologist', $transaction->radiologic_technologist) }}"
                               placeholder="Enter Radiologic Technologist Name (Optional)">
                    </div>

                    <div class="form-group">
                        <label for="date" class="form-label">Date</label>
                        <input type="date" name="date" id="date" class="form-control1"
                               value="{{ old('date', $transaction->date ? date('Y-m-d', strtotime($transaction->date)) : '') }}"
                               required>
                    </div>

                    <div class="form-group">
                        <label for="last_meal" class="form-label">Last Meal</label>
                        <input type="datetime-local" class="form-control1" id="last_meal" name="last_meal"
                               value="{{ old('last_meal', $transaction->last_meal ? date('Y-m-d\TH:i', strtotime($transaction->last_meal)) : '') }}"
                               required>
                    </div>

                    <div class="form-group">
                        <label for="total_payments" class="form-label">Total Payments</label>
                        <input type="number" step="0.01" name="total_payments" id="total_payments"
                               class="form-control1"
                               value="{{ old('total_payments', $transaction->total_payments) }}"
                               readonly>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="discount_type" id="discount_type" class="form-control1">
                            ...
                            <option value="Senior Citizen" {{ old('discount_type', $transaction->discount_type) == 'Senior Citizen' ? 'selected' : '' }}>
                                Senior Citizen (20%)
                            </option>
                            <option value="PWD" {{ old('discount_type', $transaction->discount_type) == 'PWD' ? 'selected' : '' }}>
                                PWD (20%)
                            </option>
                            <option value="Employee" {{ old('discount_type', $transaction->discount_type) == 'Employee' ? 'selected' : '' }}>
                                Student (10%)
                            </option>
                            <option value="Manual" {{ old('discount_type', $transaction->discount_type) == 'Manual' ? 'selected' : '' }}>
                                Manual Discount
                            </option>
                            <option value="None" {{ old('discount_type', $transaction->discount_type) == 'None' ? 'selected' : '' }}>
                                None
                            </option>
                        </select>
                    </div>
                    ...
                    <div id="manualDiscountSection" class="form-group"
                         style="display: {{ old('discount_type', $transaction->discount_type) == 'Manual' ? 'block' : 'none' }};">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label for="manual_discount_name" class="form-label">Discount Name</label>
                                <input type="text" name="manual_discount_name" id="manual_discount_name"
                                       class="form-control1" placeholder="Enter Discount Name"
                                       value="{{ old('manual_discount_name', $transaction->manual_discount_name) }}">
                            </div>
                            <div>
                                <label for="manual_discount_percentage" class="form-label">Discount Percentage</label>
                                <input type="number" name="manual_discount_percentage"
                                       id="manual_discount_percentage" class="form-control1"
                                       min="0" max="100" step="0.1"
                                       placeholder="Enter Discount Percentage"
                                       value="{{ old('manual_discount_percentage', $transaction->manual_discount_percentage) }}">
                            </div>
                        </div>
                        <div class="form-group mt-2">
                            <label for="manual_discount_remarks" class="form-label">Remarks</label>
                            <textarea name="manual_discount_remarks" id="manual_discount_remarks"
                                      class="form-control1" rows="2"
                                      placeholder="Enter discount remarks (optional)">{{ old('manual_discount_remarks', $transaction->manual_discount_remarks) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <div id="category{{ $category->id }}">
                                    @foreach($category->services as $service)
                                        <div class="form-check">
                                            <input class="form-check-input w-6 h-6 border-2 border-black rounded-md focus:ring-2 focus:ring-blue-500"
                                                   type="checkbox"
                                                   name="laboratory_services[]"
                                                   value="{{ $service->id }}"
                                                   {{ $transaction->services->contains($service->id) ? 'checked' : '' }}
            ...
            <!-- Drug Testing Consent Form Section -->
            <div class="section-container mb-8 bg-gray-50 rounded-2xl p-6 border border-gray-100">
                <div class="form-group">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    <div class="form-group">
                        ...
                        <select name="drug_test_purpose" id="drug_test_purpose" class="form-control1">
                            ...
                            @foreach (['Employment', 'Driver\'s License', 'Student', 'Firearms License', 'Candidate for Public Office', 'Persons Apprehended or Arrested', 'Persons Charged with Criminal Offenses', 'Others'] as $option)
                                <option value="{{ $option }}" {{ $transaction->drug_test_purpose == $option ? 'selected' : '' }}>
                                    {{ $option }}
                                </option>
                            @endforeach
                        </select>
                        <input type="text" name="other_purpose_specify" class="form-control1"
                               value="{{ old('other_purpose_specify', $transaction->other_purpose_specify) }}"
                               placeholder="Specify other purpose">
                    </div>

                    <div class="form-group">
                        <label class="form-check-label" for="medication_past_30_days">
                            Have you taken any medication/drugs in the past 30 days?
                        </label>
                    ...
                    <div class="form-group">
                        <label class="form-check-label" for="alcohol_past_24_hours">
                    ...
                    <div class="form-group">
                        ...
                        <select name="sample_type" id="sample_type" class="form-control1">
                            ...
                            @foreach (['Urine', 'Blood', 'Saliva', 'Hair', 'Sweat', 'Tissues'] as $type)
                                <option value="{{ $type }}" {{ $transaction->sample_type == $type ? 'selected' : '' }}>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-check">
                        <input class="form-check-input w-6 h-6 border-2 border-black rounded-md focus:ring-2 focus:ring-blue-500"
                               type="checkbox"
                               name="confirmatory_testing_agreement"
                               id="confirmatory_testing_agreement"
                               value="accepted"
                               {{ $transaction->confirmatory_testing_agreement == 'accepted' ? 'checked' : '' }}>
                        <label class="form-check-label" for="confirmatory_testing_agreement">
                            I hereby consent and agree that if my specimen is found positive, it will be sent to a duly accredited/licensed Confirmatory Laboratory for confirmatory testing.
                        </label</div>

                    <div class="form-check">
                        <input class="form-check-input w-6 h-6 border-2 border-black rounded-md focus:ring-2 focus:ring-blue-500"
                               type="checkbox"
                               name="sample_acknowledgement"
                               id="sample_acknowledgement"
                               value="accepted"
                               {{ $transaction->sample_acknowledgement == 'accepted' ? 'checked' : '' }}>
                        <label class="form-check-label" for="sample_acknowledgement">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Initial setup of manual discount section visibility
