...
<div class="card">
                    ...
                    <div class="queue-card">
                        <div class="flex justify-between mb-2">
                            ...
                            <div class="button-group">
                                <a href="{{ route('reception.queues.show-transaction', $pendingQueue->transaction_id) }}" class="btn btn-info btn-sm">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <form action="{{ route('reception.queues.start-process', $pendingQueue->id) }}" method="POST" style="display:inline;">
                                    @csrf
                ...
                @forelse($queues->where('status', 'in_progress')->take(4) as $progressQueue)
                    <div class="queue-card">
                        <div class="flex justify-between mb-2">
                            ...
                            <div class="button-group">
                                <form action="{{ route('reception.queues.markAsDone', $progressQueue->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                ...
                @forelse($queues->where('status', 'done')->sortByDesc('updated_at')->take(4) as $doneQueue)
                    <div class="queue-card">
                        <div class="flex justify-between mb-2">
                            ...
                            <div class="button-group">
                                <form action="{{ route('reception.queues.release', $doneQueue->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('PUT')
    ...
    <a href="{{ route('reception.queue.display') }}" class="btn btn-success">View Queue Display</a>
</div>
...
<script>
    ...
    function updateServiceStatus(queueId, serviceId, completed) {
        fetch(`/admin/queues/${queueId}/update-service`, {
            method: 'POST',
            headers: {
