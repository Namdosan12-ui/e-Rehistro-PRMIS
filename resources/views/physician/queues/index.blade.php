                    ...
                    <div class="queue-card">
                        <div class="flex justify-between mb-2">
                            ...
                            <div class="button-group">
                                <a href="{{ route('physician.queues.show-transaction', $pendingQueue->transaction_id) }}" class="btn btn-info btn-sm">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <form action="{{ route('physician.queues.start-process', $pendingQueue->id) }}" method="POST" style="display:inline;">
                                    @csrf
                ...
                @forelse($queues->where('status', 'in_progress')->take(4) as $progressQueue)
                    <div class="queue-card">
                        <div class="flex justify-between mb-2">
                            ...
                            <div class="button-group">
                                <form action="{{ route('physician.queues.markAsDone', $progressQueue->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                ...
                @forelse($queues->where('status', 'done')->sortByDesc('updated_at')->take(4) as $doneQueue)
                    <div class="queue-card">
                        <div class="flex justify-between mb-2">
                            ...
                            <div class="button-group">
                                <form action="{{ route('physician.queues.release', $doneQueue->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('PUT')
