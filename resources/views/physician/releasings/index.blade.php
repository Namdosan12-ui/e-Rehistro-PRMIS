                    ...
                    <tr>
                        ...
                        <td>
                            @if ($releasing->result_file)
                                <a href="{{ route('physician.releasings.view', $releasing->id) }}" target="_blank">View Result</a>
                            @else
                                No result uploaded
                            @endif
                        </td>
                        <td>
                            @if (!$releasing->result_file)
                                <form action="{{ route('physician.releasings.upload', $releasing->id) }}" method="POST" enctype="multipart/form-data" class="upload-form">
                                    @csrf
                            ...
                            @elseif ($releasing->result_file && !$releasing->released_at)
                                @include('physician.partials.email_form', ['releasing' => $releasing])
                            @endif
                        </td>
                        <td>
                            @if (!$releasing->released_at)
                                @include('physician.partials.release_form', ['releasing' => $releasing])
                            @else
